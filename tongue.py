...
class History:
    ...
    def record_score(self, difficulty, score):
        ...
        if difficulty == self.difficulty:
            ...
            if DEBUG:
                print(f"[DEBUG] record_score: recorded score {score} at level {difficulty}, scores now: {self.level_scores}")
            ...
            if DEBUG:
                print(f"[DEBUG] record_score: SKIPPED score {score} - round difficulty {difficulty} != current difficulty {self.difficulty}")
    ...
    def check_advancement(self):
        ...
        if self.difficulty >= MAX_DIFFICULTY:
            if DEBUG:
                print(f"[DEBUG] check_advancement: at max difficulty {self.difficulty}, cannot advance")
            return False
        good_count = self.get_good_score_count()
        if DEBUG:
            print(f"[DEBUG] check_advancement: level={self.difficulty}, scores={len(self.level_scores)}/{ADVANCE_WINDOW_SIZE}, good={good_count}/{ADVANCE_REQUIRED_GOOD}")
        if len(self.level_scores) >= ADVANCE_WINDOW_SIZE:
            if good_count >= ADVANCE_REQUIRED_GOOD:
                if DEBUG:
                    print(f"[DEBUG] check_advancement: ADVANCING! scores={self.level_scores}")
                return True
        return False
    ...
    def get_progress_display(self):
        """Get a string showing current level and progress toward next level."""
        good_count = self.get_good_score_count()
        if self.difficulty >= MAX_DIFFICULTY:
            return f"Level {self.difficulty} (MAX)"
        return f"Level {self.difficulty} → {self.difficulty + 1}: {good_count}/{ADVANCE_REQUIRED_GOOD}"
    ...
    def evaluate_round(self, round, chat, max_retries=3):
        ...
        for attempt in range(max_retries):
            ...
            # All retries failed
            print(f'\nError: Failed to validate translation after {max_retries} attempts.')
            print(f'Last error: {last_error}')
            print('Your translation has been saved. Try again or type "exit" to quit.')
            save_state()
            return False
        ...
        if DEBUG:
            print(f"[DEBUG] Before level check: difficulty={self.difficulty}, round.difficulty={round.difficulty}")
...
def validate_response(sentence, translation, chat):
    ...
    """
    try:
        judgement_raw, ms = execute_chat(chat, prompt_judge)
    except Exception as e:
        raise RuntimeError(f"Gemini API error while validating translation: {e}")

    try:
        judgement = sanitize_judgement(judgement_raw)
        judgement = eval(judgement)
    except Exception as e:
        if DEBUG:
            print(f"[DEBUG] Raw response from Gemini:\n{judgement_raw}")
        raise RuntimeError(f"Failed to parse Gemini response: {e}\nRaw response (first 500 chars): {judgement_raw[:500]}")

    # Validate expected fields
    required_fields = ['vocabulary_breakdown', 'correct_translation', 'score', 'evaluation']
    missing = [f for f in required_fields if f not in judgement]
    if missing:
        raise RuntimeError(f"Gemini response missing required fields: {missing}\nGot: {list(judgement.keys())}")
...
def main():
    ...
    while True:
        ...
        if not history.evaluate_round(round, chat):
            # Validation failed - put sentence back for retry
            history.story_sentences.insert(0, round.sentence)
            history.rounds.pop()  # Remove the failed round
            save_state()
